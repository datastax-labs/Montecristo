...
class CompactionTest {
    ...
    fun getDocumentRecommendChangeToLCS() {
        ...
        assertThat(recs[0].longForm).isEqualTo("We recommend upgrading to >=5.0 or HCD and using compaction strategy UCS on 1 table, otherwise before 5.0 evaluate changing to LCS.")
    ...
    fun getDocumentRecommendChangeToTWCS() {
        ...
        assertThat(recs[0].longForm).isEqualTo("We recommend upgrading to >=5.0 or HCD and using compaction strategy UCS on 1 table, otherwise before 5.0 evaluate changing to TWCS.")
        assertThat(recs[1].longForm).isEqualTo("There is 1 table with the p95 SSTable Per Read histogram above 5. This indicates an issue in the data model or compaction strategy choice for the table. We recommend reviewing the table, usage and compaction strategy.")
    ...
    fun getDocumentRecommendChangeToTWCSFromDTCSTables() {
        ...
        val template = compaction.getDocument(cluster, searcher, recs, ExecutionProfile.default())
        assertThat(recs.size).isEqualTo(2)
        assertThat(recs[0].longForm).isEqualTo("We recommend upgrading to >=5.0 or HCD and using compaction strategy UCS on 2 tables, otherwise before 5.0 evaluate changing to TWCS.")
