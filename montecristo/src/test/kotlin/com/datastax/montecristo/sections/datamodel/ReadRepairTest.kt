...
internal class ReadRepairTest {

    @Test
    fun getDocumentNoRepairActive() {

        val table = mockk<Table>(relaxed = true)
        every { table.name } returns "test_table1"
        every { table.dcLocalReadRepair } returns "0.0"
        every { table.readRepair } returns "0.0"
    ...
    @Test
    fun getDocumentLocalRepairActive() {

        val table = mockk<Table>(relaxed = true)
        every { table.name } returns "test_table1"
        every { table.dcLocalReadRepair } returns "0.2"
        every { table.readRepair } returns "0.0"
    ...
    @Test
    fun getDocumentLocalRepairActiveUsingDefault() {
    ...
    @Test
    fun getDocumentGlobalRepairActive() {
        ...
        assertThat(recs.size).isEqualTo(1)
        assertThat(recs[0].priority).isEqualTo(RecommendationPriority.IMMEDIATE)
        assertThat(recs[0].longForm).contains("Tables are using global (multi-dc) read repair which can add significant overhead.")
    }

    @Test
    fun getDocumentNoReadRepairSpecified() {

        val table = mockk<Table>(relaxed = true)
        every { table.name } returns "test_table1"
        every { table.dcLocalReadRepair } returns ""
        every { table.readRepair } returns ""
