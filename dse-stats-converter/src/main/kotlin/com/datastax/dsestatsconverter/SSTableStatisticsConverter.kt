        ...
        files.map { file ->
            try {
                if (file.name.endsWith(".db")) {
                    // this has to be a private call, the main method call has a system.exit(0) in it which kills the whole process.
                    metawriter.callPrivateFunc("printSStableMetadata", file.absolutePath, false)
                    capture.flush()
                    // write output
